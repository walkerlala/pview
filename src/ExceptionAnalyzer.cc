...
bool ExceptionAnalyzer::ExceptionInfo::filterByCatch(const Type *BaseClass) {
  ...
  for (const Type *T : ThrownExceptions) {
    if (T == BaseClass || isBaseOf(T, BaseClass)) TypesToDelete.push_back(T);
  }

  for (const Type *T : TypesToDelete) ThrownExceptions.erase(T);
...
ExceptionAnalyzer::ExceptionInfo ExceptionAnalyzer::throwsException(
    ...
    int64_t CurLevel) {
  if (CallStack.count(Func)) return ExceptionInfo::createNonThrowing();
  if (CurLevel >= MaxTraceLevel) return ExceptionInfo::createNonThrowing();

  int64_t NextLevel = CurLevel + 1;
...
ExceptionAnalyzer::ExceptionInfo ExceptionAnalyzer::throwsException(
    ...
    llvm::SmallSet<const FunctionDecl *, 32> &CallStack, int64_t CurLevel) {
  ...
  if (const auto *Throw = dyn_cast<CXXThrowExpr>(St)) {
    ...
    ExceptionInfo Uncaught =
        throwsException(Try->getTryBlock(), Caught, CallStack, NextLevel);
    ...
    if (const FunctionDecl *Func = Call->getDirectCallee()) {
      ExceptionInfo Excs = throwsException(Func, CallStack, NextLevel);
      Results.merge(Excs);
    }
  } else if (const auto *Construct = dyn_cast<CXXConstructExpr>(St)) {
    ExceptionInfo Excs =
        throwsException(Construct->getConstructor(), CallStack, NextLevel);
    Results.merge(Excs);
  } else if (const auto *DefaultInit = dyn_cast<CXXDefaultInitExpr>(St)) {
    ExceptionInfo Excs =
        throwsException(DefaultInit->getExpr(), Caught, CallStack, NextLevel);
    Results.merge(Excs);
  } else {
...
ExceptionAnalyzer::ExceptionInfo ExceptionAnalyzer::analyzeImpl(
    const FunctionDecl *Func) {
  ...
  if (FunctionCache.count(Func) == 0) {
    llvm::SmallSet<const FunctionDecl *, 32> CallStack;
    ExceptionList = throwsException(Func, CallStack, /*CurLevel*/0);
...
template <typename T>
ExceptionAnalyzer::ExceptionInfo ExceptionAnalyzer::analyzeDispatch(
    const T *Node) {
...
ExceptionAnalyzer::ExceptionInfo ExceptionAnalyzer::analyze(
    const FunctionDecl *Func) {
