...
namespace pview {
/// This class analysis if a `clang::FunctionDecl` can in principle throw an
...
class ExceptionAnalyzer {
 public:
  ...
  class ExceptionInfo {
   public:
    using Throwables = llvm::SmallSet<const clang::Type *, 2>;
    ...
    /// thrown.
    void registerException(const clang::Type *ExceptionType);
    ...
    bool filterByCatch(const clang::Type *BaseClass);
    ...
    ExceptionInfo &filterIgnoredExceptions(
  ...
  void setMaxTraceLevel(int64_t Level) { MaxTraceLevel = Level; }
  ...
  ExceptionInfo analyze(const clang::FunctionDecl *Func);
  ExceptionInfo analyze(const clang::Stmt *Stmt);

 private:
  ExceptionInfo throwsException(
      const clang::FunctionDecl *Func,
      llvm::SmallSet<const clang::FunctionDecl *, 32> &CallStack,
      int64_t CurLevel);
  ExceptionInfo throwsException(
      const clang::Stmt *St, const ExceptionInfo::Throwables &Caught,
      llvm::SmallSet<const clang::FunctionDecl *, 32> &CallStack,
      int64_t CurLevel);

  ExceptionInfo analyzeImpl(const clang::FunctionDecl *Func);
  ExceptionInfo analyzeImpl(const clang::Stmt *Stmt);

  template <typename T>
  ExceptionInfo analyzeDispatch(const T *Node);

  int64_t MaxTraceLevel = -1;
  bool IgnoreBadAlloc = true;
  llvm::StringSet<> IgnoredExceptions;
  std::map<const clang::FunctionDecl *, ExceptionInfo> FunctionCache;
};
